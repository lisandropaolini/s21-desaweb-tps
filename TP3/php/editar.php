...
try {
  if (!isset($_POST['submit'])) {
    ...
    $alf_cer = $_POST['tabla-adc'];
    $alf_arr = $_POST['tabla-ada'];
    $mix_cer = $_POST['tabla-mdc'];
    $estado = "";

    if ($_POST['EstadoSwitchCheck'] == 'on'){
      $estado = "ENTREGADO";
      $fecha_entregado = "SYSDATE()";
    } else {
      $estado = "INGRESADO";
    ...
    $dsn = 'mysql:host=localhost;dbname=lisandro_tp3';
    $conexion = new PDO($dsn, "mysql", "", [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
    $consultaSQL = "UPDATE pedidos SET
      nombre= '$nombre',
      email= '$email',
      telefono= '$telefono',
      direccion= '$direccion',
    ...
    <div class="col-md-6" style="margin-top: 20px;">
      <div class="card">
        <div class="card-header text-center">
          ...
          <form action="editar.php" method="post" class="text-start">
            <input type="hidden" id="id_pedidos" name="id_pedidos" value="<?= $_GET['id'] ?>"/>
            <label for="cliente" class="form-label">Detalle del pedido</label>
            <div class="mb-3">
              <input type="text" class="form-control" id="nombre" name="nombre"
                placeholder="Nombre" value="<?= $pedido['nombre'] ?>" required>
            </div>
            <div class="mb-3">
              <input type="email" class="form-control" id="email"  name="email"
               placeholder="Email" value="<?= $pedido['email'] ?>" required>
            </div>
            <div class="mb-3">
              <input type="tel" class="form-control" id="telefono" name="telefono"
               placeholder="Teléfono" value="<?= $pedido['telefono'] ?>" required>
            </div>
            <div class="mb-3">
              <input type="text" class="form-control" id="direccion" name="direccion"
               placeholder="Dirección" value="<?= $pedido['direccion'] ?>" required>
            </div>
              ...
              <tbody>
                <tr>
                  ...
                  <td><input type="number" min="0" max="20" id="tabla-adc"
                   ...
                   value="<?= $pedido['alf_cer'] ?>" required></td>
                </tr>
                <tr>
                  ...
                  <td><input type="number" min="0" max="20" id="tabla-ada"
                   ...
                   value="<?= $pedido['alf_arr'] ?>" required></td>
                </tr>
                <tr>
                  ...
                  <td><input type="number" min="0" max="20" id="tabla-mdc"
                   ...
                   value="<?= $pedido['mix_cer'] ?>" required></td>
                </tr>
                <tr>
                  <td>Estado</td>
                  <td>
                  <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="EstadoSwitchCheck"
                      name="EstadoSwitchCheck" <?php if ($pedido['estado'] == "ENTREGADO") {echo "checked";} ?>>
            ...
            <a href="index.php" class="text-decoration-none">
              ...
              <button type="submit" name="submit" class="btn btn-primary">Editar pedido</button>
            </div>
          </form>
        </div>
      </div>
